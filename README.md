...
A browser-based tool for Bloomsbury Publishing that converts journal/book subscription order data from Excel spreadsheets into the fixed-width EDI format required by the Taylor & Francis / BLOUK distribution system.
...
The tool accepts an `.xlsx`, `.xls` or `.csv` file containing subscription order rows and outputs a plain-text EDI file (`.txt`) in the BLOUK T1 fixed-width format. No server, backend, or installation is required — it runs entirely in the browser.
...
```
blouk_edi/├── index.html   — Application markup (HTML only, no inline scripts or styles)├── style.css    — All visual styling and layout├── script.js    — All application logic└── README.md    — This document
```
...
3.  An internet connection is required on first load to fetch CDN libraries (SheetJS, Font Awesome, Google Fonts). Subsequent use may work offline if the browser has cached those resources.
...
Drag your spreadsheet onto the upload zone, or click it to open the file picker. Accepted formats: `.xlsx`, `.xls`, `.csv`.
...
-   Enables the **Copy** and **Download .txt** buttons

### Step 5 — Download

-   **Download .txt** — saves the file to your downloads folder with the standard filename convention.
...
Trailing space is significant for auto-match

`ISSN`

ISBN/ISSN (D1)

Normalised to 13 digits

`Journal/ Issue Title`

Title

Not written to EDI; for reference only

`Quantity`

D1 quantity

Falls back to Default Qty setting if blank
...
Accepts ISO 2-letter or 3-letter codes
...
Rows that share the same **Subscription number**, **Delivery Company** and **Delivery Name** are automatically grouped into a single order with multiple D1 line items. Rows with no Subscription number are each treated as a separate single-line order.

---

## EDI Output Format

The generated file is a fixed-width ASCII text file with CRLF (`rn`) line endings. All fields occupy exact character positions with no delimiters. Unused space within a field is padded with spaces (text fields) or zeros (numeric fields).

### File Structure

```
$$HDR{sender}  {fileId}   {timestamp}H1...  (350 chars)H2...  (358 chars)H3...  ( 20 chars)D1...  (266 chars)   ← one per line item[repeat H1/H2/H3/D1 blocks for each order]$$EOF{sender}  {fileId}   {timestamp}{recordCount}
```
...
Customer code (`ST` + subscription ref)
...
Country code (ISO alpha-3, e.g. `GBR`)
...
Item reference (subscription number)

42

5

Line number zero-padded (`00001`)
...
Quantity block (`0000001` + 21 zeros)

174

12

Unit price in pence (space + 9 digits + 2 spaces); set to 0
...
Record count (7 digits, zero-padded) — H1+H2+H3+D1 lines only
...
File ID / Batch Number

`0027816`

Appears in $$HDR and $$EOF markers. Update per batch if required.

File Prefix

`T1`

Used in the output filename only (e.g. `T1.0027816_...`).

Sender Code

`BLOUK`

4–6 character code written to $$HDR / $$EOF.

Currency

`GBP`

3-character currency code written to each H1 record.

Payment Terms

`FCA`

Incoterms code written to each H3 record. `FCA` = Free Carrier; `DAP` = Delivered at Place.

Order Number Start

Auto

Read-only. Timestamp-derived; regenerated on each Generate click.

Default Qty

`1`

Fallback quantity used when the Quantity column is blank or zero.
...
This matches the convention observed in reference files from the distribution system (`T1.M02221600__22-02-26_.txt`).
...
[IBM Plex Sans + Mono](https://fonts.google.com/specimen/IBM+Plex+Sans)
...
The Clipboard API (`navigator.clipboard`) used by the Copy button requires HTTPS or localhost. On plain HTTP, the copy button will silently fail; use Download instead.
...
-   **Same-minute duplicates** — If two batches are generated within the same minute, their order number ranges will share the same seed. In normal use this is not a practical concern.
...
3.  Update the README field tables.
